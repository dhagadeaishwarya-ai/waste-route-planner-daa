...
void initCity() {
    ...
    vector<string> names = {
        "AZAM_NAGAR_CIRCLE","AZAM_NAGAR_1ST_CROSS","AZAM_NAGAR_2ND_CROSS",
        "AZAM_NAGAR_3RD_CROSS","AZAM_NAGAR_4TH_CROSS","AZAM_NAGAR_5TH_CROSS",
        "CROSS_6","CROSS_7","CROSS_8",
        "JUNCTION_0","JUNCTION_1","JUNCTION_2","JUNCTION_3","JUNCTION_4","JUNCTION_5",
        "DHABA_POINT","GARDEN","DAMRO","STEAM_OFFICE","TRAINING_CENTER",
        "KALMESHWAR","SAMATH","DATT","VANDAN_COLONY",
        "DUMPING_YARD"
    };
...
/**********************************************************************************************
ZONE MANAGEMENT OPERATIONS (INSERT, UPDATE, SEARCH)
**********************************************************************************************/
void zoneManagement(){
    int choice;
    do {
        ...
        switch(choice) {
            case 1: {
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
                cout << CYAN << "Enter zone name/ID to search: " << RESET;
                string input;
                getline(cin, input);
                ...
                try {
                    ...
                    if(zoneId >= 0 && zoneId < zones.size()) {
                        cout << GREEN << "\nZone Found:\n" << RESET;
                        cout << "ID: " << zones[zoneId].id << "\n";
                        cout << "Name: " << zones[zoneId].name << "\n";
                        cout << "Waste Level: " << zones[zoneId].currentWaste << "%\n";
                        cout << "Priority: " << zones[zoneId].priority << "\n";
                    } else {
            ...
            case 2: {
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
                cout << CYAN << "Enter zone name to update: " << RESET;
                string input;
                getline(cin, input);
                ...
                int zid = matches[0];
                cout << "Current waste level: " << zones[zid].currentWaste << "%\n";
                ...
                if(newWaste >= 0 && newWaste <= 100) {
                    ...
                    cout << GREEN << "Zone updated successfully!\n" << RESET;
                } else {
                    cout << RED << "Invalid waste level. Must be between 0 and 100.\n" << RESET;
                }
                break;
            }
            case 3: {
                ...
                newZone.history = {rand()%100, rand()%100, newZone.currentWaste};
                newZone.x = rand()%100;
                newZone.y = rand()%100;
                newZone.priority = newZone.currentWaste;
                
                zones.push_back(newZone);
                id[name] = newId;
                lookupTable.insert(newId, name);
                
                // Update adjacency list and distance matrix
                adj.push_back({});
                dist.resize(zones.size());
                for(int i=0; i<zones.size(); i++) {
                    dist[i].resize(zones.size(), INF);
                }
                for(int i=0; i<zones.size(); i++) dist[i][i] = 0;
                
                cout << GREEN << "New zone added successfully with ID: " << newId << "\n" << RESET;
                cout << YELLOW << "Note: You may need to add road connections manually.\n" << RESET;
                break;
            }
            case 4: {
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
                cout << CYAN << "Enter zone name: " << RESET;
                string input;
                getline(cin, input);
                ...
                cout << "ID: " << z.id << "\n";
                cout << "Name: " << z.name << "\n";
                cout << "Current Waste: " << z.currentWaste << "%\n";
                cout << "Priority: " << z.priority << "\n";
                cout << "Coordinates: (" << z.x << ", " << z.y << ")\n";
                cout << "History Size: " << z.history.size() << " entries\n";
                if(!z.history.empty()) {
                    cout << "Recent History: ";
                    int start = max(0, (int)z.history.size() - 5);
                    for(int i=start; i<z.history.size(); i++) {
                        cout << z.history[i] << "% ";
                    }
                    cout << "\n";
                }
                cout << "Connected Zones: " << adj[zid].size() << "\n";
                cout << "=====================================\n" << RESET;
                break;
            }
            case 5: {
...
void advancedRouteOptimization(){
    ...
    vector<int> startMatches = lookupTable.findAllMatches(startInput);
    
    if(startMatches.empty()) {
        cout << RED << "Starting zone not found.\n" << RESET;
        return;
    }
    
    int start = startMatches[0];
    
    cout << CYAN << "Select optimization strategy:\n" << RESET;
    ...
    // Get target zones (zones with waste > 50%)
    vector<int> targets;
    for(int i=0; i<zones.size(); i++) {
    ...
    if(targets.empty()) {
        cout << YELLOW << "No high-priority zones found.\n" << RESET;
        return;
    }
    ...
    if(strategy == 1 || strategy == 3) {
        vector<int> targetsCopy1 = targets;
        Route route1 = optimizedRouteGreedy(start, targetsCopy1);
        
        cout << GREEN << "\n--- GREEDY NEAREST NEIGHBOR ROUTE ---\n" << RESET;
        cout << "Total Distance: " << route1.totalDistance << " km\n";
        cout << "Total Cost: Rs. " << fixed << setprecision(2) << route1.totalCost << "\n";
        cout << "Total Waste Collected: " << route1.totalWaste << " units\n";
        cout << "Stops: " << route1.path.size() << "\n";
    }
    
    if(strategy == 2 || strategy == 3) {
        vector<int> targetsCopy2 = targets;
        Route route2 = optimizedRoutePriority(start, targetsCopy2);
        
        cout << MAGENTA << "\n--- PRIORITY-BASED ROUTE ---\n" << RESET;
        cout << "Total Distance: " << route2.totalDistance << " km\n";
        cout << "Total Cost: Rs. " << fixed << setprecision(2) << route2.totalCost << "\n";
        cout << "Total Waste Collected: " << route2.totalWaste << " units\n";
        cout << "Stops: " << route2.path.size() << "\n";
        ...
        if(strategy == 3) {
            ...
            if(route1.totalDistance < route2.totalDistance) {
                cout << "Greedy route is shorter by " << (route2.totalDistance - route1.totalDistance) << " km\n";
            } else {
                cout << "Priority route is shorter by " << (route1.totalDistance - route2.totalDistance) << " km\n";
            }
            
            if(route1.totalCost < route2.totalCost) {
                cout << "Greedy route saves Rs. " << fixed << setprecision(2) << (route2.totalCost - route1.totalCost) << "\n";
            } else {
                cout << "Priority route saves Rs. " << fixed << setprecision(2) << (route1.totalCost - route2.totalCost) << "\n";
            }
        }
    }
...
void historicalDataAnalysis(){
    ...
    if(z.history.size() < 2) {
        cout << YELLOW << "Insufficient historical data for analysis.\n" << RESET;
        return;
    }
    ...
    double trend = 0;
    if(z.history.size() >= 2) {
        trend = (double)(z.history.back() - z.history[0]) / (z.history.size() - 1);
    }
    
    cout << GREEN << "\n========== HISTORICAL ANALYSIS: " << z.name << " ==========\n" << RESET;
    cout << "Total Data Points: " << z.history.size() << "\n";
    cout << "Minimum Value: " << minVal << "%\n";
    cout << "Maximum Value: " << maxVal << "%\n";
    cout << "Average Value: " << fixed << setprecision(2) << avg << "%\n";
    cout << "Current Value: " << z.currentWaste << "%\n";
    cout << "Overall Trend: " << (trend > 0 ? "+" : "") << fixed << setprecision(2) << trend << "% per period\n";
    
    if(segmentTrees[zid] != nullptr) {
        int maxRecent = segmentTrees[zid]->getMax();
        cout << "Maximum in History (Segment Tree): " << maxRecent << "%\n";
    }
    
    cout << "\n" << CYAN << "RECENT HISTORY (Last 10 entries):\n" << RESET;
    int start = max(0, (int)z.history.size() - 10);
    for(int i = start; i < z.history.size(); i++) {
        cout << "Day " << (i - start + 1) << ": " << z.history[i] << "%\n";
    }
...
void batchZoneUpdate(){
    ...
    for(int i = 0; i < count; i++) {
        ...
        vector<int> matches = lookupTable.findAllMatches(name);
        if(matches.empty()) {
            cout << YELLOW << "Zone not found, skipping...\n" << RESET;
            continue;
        }
        ...
        if(newWaste >= 0 && newWaste <= 100) {
            ...
            cout << RED << "Invalid waste level, skipping...\n" << RESET;
        }
    }
...
void routeEfficiencyCalculator(){
    ...
    double avgWastePerKm = (double)r.totalWaste / max(1, r.totalDistance);
    double costPerWaste = r.totalCost / max(1, r.totalWaste);
    double efficiency = (double)r.totalWaste / max(1, r.totalCost); // waste per rupee
        ...
        switch(choice){
            ...
            case 9:
                cout << MAGENTA << "Google Maps Example Route:\n"
                     << "https://www.google.com/maps/dir/AZAM_NAGAR_CIRCLE/GARDEN/DAMRO/DUMPING_YARD"
                     << RESET << "\n";
                break;
